...
export const calculateWeightHandlingFee = (
  ...
  if (mode === "Easy Ship") {
    ...
    if (size === "Standard" && "first500g" in sizeFees) {
      if (weight <= 0.5) {
  ...
  if (mode === "FBA") {
    const fees =
      weightHandlingFees.fba.standard[
        serviceLevel.toLowerCase() === "advanced"
          ? "premium"
          : serviceLevel.toLowerCase()
      ];
...
export const storageFees = (): number => {
  return 45;
};
